...
class ParsedBean {
  ...
  WriterModel detect() {
    print('in detect, begin parse and generate...');
    _getModel();
  ...
  void _getModel() {
    ...
    print('clazz.metadata = ${clazz.metadata}');
    // ElementAnnotation - 表示注释元素
    // metadata 表示类上方的所有的 @类开头的
    // [@Column({String name, bool isNullable: false, String uniqueGroup, bool auto: false, int length}) → Column, @GenBean({Map<String, ColumnBase> columns: const <String, ColumnBase> {}, Map<String, Relation> relations: const <String, Relation> {}}) → GenBean]
    ...
    print('meta.computeConstantValue()= ${meta.computeConstantValue()}');
    //GenBean (columns = Map<String, ColumnBase> ({}); relations = Map<String, Relation> ({}))
    ...
    //interface.typeArguments 表示尖括号中的内容参数（泛型内容），
    print('interface.typeArguments = ${interface.typeArguments}');
    //interface.typeArguments = [Post]

    // model 表示泛型内容类型，这里是Post
  ...
  void _parseFields() {
    ...
    print('modelClass.fields= ${modelClass.fields}');
